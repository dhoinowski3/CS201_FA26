...
```{r, messages=FALSE}
unique(penguins$sex)# an example of how to use unique()

female_culmen <- filter(penguins, sex == "FEMALE", culmen_length_mm >= 39)

unique(female_culmen$sex)
...
alter_island_name <- penguins |> 
  rename(island_name = island)

view(alter_island_name)
```
...
final_boss <- penguins |> 
  filter(penguins, sex == "FEMALE", culmen_length_mm >= 39, na.rm)
